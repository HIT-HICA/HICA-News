...
link = os._exit(-1) if link==[] else link[0].replace('amp;','')
updateTime = re.findall("(?<=timeConvert.{2}).*?(?='\))",res.text)
updateTime = os._exit(-2) if updateTime==[] else time.strftime("%Y-%m-%d", time.localtime(int(updateTime[0])))
...
msg = os._exit(-4) if msg==[] else json.loads(msg[0])
...
for url in urlList:
	#using the same method
	#url is already given
	#using the same  headers
	res = requests.request(url=url,method=method,headers=headers)
	
	content = re.findall('<section.*',res.text)
	content = os._exit(-5) if content==[] else content[0].replace('data-src','src')
	with open('articles/'+str(msgList[-1]['id']),'w') as f:
